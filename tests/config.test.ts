...
test("resolveEnvVars › resolves existing env vars with $VAR syntax", (t) => {
	...
	t.is(resolveEnvVars("Prefix $TEST_VAR_1 Suffix"), "Prefix test_value_1 Suffix");
});

test("resolveEnvVars › resolves existing env vars with ${VAR} syntax", (t) => {
	process.env.TEST_VAR_2 = "test_value_2";
	t.is(resolveEnvVars("${TEST_VAR_2}"), "test_value_2");
	t.is(resolveEnvVars("Prefix ${TEST_VAR_2} Suffix"), "Prefix test_value_2 Suffix");
});
...
test.serial("getPlatformConfigDir › generates correct paths based on platform", (t) => {
	const originalPlatform = process.platform;
	
	Object.defineProperty(process, 'platform', { value: 'darwin' });
	t.is(getPlatformConfigDir("testapp"), path.join(os.homedir(), "Library", "Preferences", "testapp"));
	
	Object.defineProperty(process, 'platform', { value: 'linux' });
	t.is(getPlatformConfigDir("testapp"), path.join(os.homedir(), ".config", "testapp"));
